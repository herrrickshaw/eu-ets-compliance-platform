...
def list_projects(country: str | None = None, db: Session = Depends(get_db)):
...
def purchase_unit(unit_id: int, req: schemas.CreditPurchaseRequest, db: Session = Depends(get_db)):
...
def retire_unit(unit_id: int, db: Session = Depends(get_db)):
    unit = db.query(models.CreditUnit).get(unit_id)
    if unit is None:
        raise HTTPException(404, "Unit not found")
